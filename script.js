...
(() => {
 const sampleMenu = [
  ...
  const getCart = () => JSON.parse(localStorage.getItem('Goshop_cart') || '[]');
  const saveCart = (cart) => localStorage.setItem('Goshop_cart', JSON.stringify(cart));
  ...
  function renderMenu(){
    ...
    sampleMenu.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <h3>${item.name}</h3>
        <p class="muted">Just a door away.</p>
